...
لینک‌های سابسکریپشن هر ۲ روز یکبار با جستجو در ریپازیتوری‌های گیت‌هاب مرتبط با ایران یافت می‌شوند ( فقط لینک‌هایی که در۵ روز گذشته بروز شده باشند انتخاب می‌گردند). ( بنابراین همیشه لینکهای جدید جایگزین لینکهای قدیمی میشوند)

کانفیگ‌های درون همه لینکها استخراج شده و همه کانفیگ‌های تکراری حذف می‌شوند سپس تمام کانفیگ‌های باقیمانده تست می‌شوند و در نهایت **۲۰۰۰ تا از سریعترین کانفیگها** در فایل `Triton-ix.txt` ذخیره می‌گردد تا شما بتوانید به‌راحتی از آنها استفاده کنید.
